...
function backgroundElements(x, y, s) {
  ...
  strokeWeight();
  fill(246, 239, 233);
  ellipse(800 / 2, 800 / 2 + 100, 700);
  fill(241, 227, 215);

  translate(800 / 2 - 160, 800 / 2 - 120);
  rotate(2.5);
  ellipse(0, 0, 140, 80);
  pop();
  ...
  quad(
    x - 135 * s, y - 40 * s, x - 120 * s, y + 200 * s, x + 120 * s, y + 200 * s, x + 110 * s, y - 60 * s);
...
function victimStickman(x, y, s) {
  ...
  quad( x - 85 * s, y - 60 * s, x - 50 * s, y - 20 * s, x + 50 * s, y - 20 * s, x + 85 * s, y - 60 * s ); pop();
...
function draw() {
  ...
  if (gameState === "start") {
    ...
    textSize(20);
    fill(255, 255, 255);
    text("Saving Steve from a haunted house!", 50, 50);
    textSize(13);
    text( "Steve and Alex were trapped inside a haunted house. Alex was able to run away, but Steve is trapped inside the elevator. Help Alex save Steve and stay alive!", 50, 80, 500, 80 );
    text("Press [SPACE] to contol the switch ", 50, 137);
    text("Click the mouse to start the game", 50, 167);
    pop();
  } else if (gameState === "game") {
    // the setup of the game screen
    ...
    if (gameRunning) {
      ...
      if (victim.y > landingY && victim.velocity > 3) {
        gameRunning = false;
        gameState = "end";
        result = "You killed Steve :( Be more careful next time!";
    ...
    if (result.includes("killed")) {
      backgroundElements(400, 400, 1);
      ground();
      victimStickmanNoElevator(650, 600, 0.7);

      push();
      fill(255, 255, 255);
      text("To restart click the mouse", 50, 130);
      pop();
